...
async function seedCustomers() {
  ...
  const customers = [
    ...
    },
    {
      ...
      phone: "+2348044444444",
      dob: "1979-07-30",
      license: "NG-PH-003456",
      address: "17 Rumuola Rd",
      city: "Port Harcourt",
      state: "Rivers",
      postalCode: "500001",
      country: "Nigeria",
    },
    {
      ...
      phone: "+2348055555555",
      dob: "1993-01-14",
      license: "NG-AB-007890",
      address: "9 Wuse Market",
      city: "Abuja",
      state: "FCT",
      postalCode: "900002",
      country: "Nigeria",
  ...
  for (const v of vehicles) {
    const vehicle = await prisma.vehicle.upsert({
      ...
      create: {
        ...
        warrantyExpiresAt: new Date("2026-12-31"),
      },
    });
    result.push(vehicle);
  }
...
async function seedAppointmentsAndJobCards(
  ...
  for (const a of apptData) {
    ...
    const appt =
      existing ?? (await prisma.serviceAppointment.create({ data: a }));
    appts.push(appt);
  }
...
async function seedJobCards(
  ...
  const tech3 = staffUsers.find((u) => u.email === "tech3@drivecare.com")!;
  const tech4 = staffUsers.find((u) => u.email === "tech4@drivecare.com")!;
  const advisor1 = staffUsers.find(
    (u) => u.email === "advisor1@drivecare.com",
  )!;
  ...
  const jobCardDefs = [
    {
      ...
      vehicleId: v0.id,
      branchId: mainBranch.id,
      description: "Full service — oil change, brake inspection, air filter",
      status: "Closed",
      estimatedHours: 2,
      estimatedCost: 35000,
      technicianId: tech1.id,
      qualityInspectorId: tech2.id,
      progress: 100,
      qcStatus: "Passed",
    },
    {
      jobNumber: "JC-2026-002",
      appointmentId: appts[1].id,
      customerId: c0.customer.id,
      vehicleId: v1.id,
      branchId: abujaBranch.id,
      description: "Oil change and 20-point inspection",
      status: "Closed",
      estimatedHours: 1.5,
      estimatedCost: 22000,
    ...
    {
      jobNumber: "JC-2026-003",
      appointmentId: appts[2].id,
      customerId: c1.customer.id,
      vehicleId: v2.id,
      branchId: mainBranch.id,
      description: "Routine service — oil, filters, tyre pressure",
      status: "Closed",
      estimatedHours: 1,
      estimatedCost: 18500,
    ...
    {
      jobNumber: "JC-2026-004",
      appointmentId: appts[3].id,
      customerId: c1.customer.id,
      vehicleId: v3.id,
      branchId: phBranch.id,
      description: "Tyre rotation, wheel balancing, alignment check",
      status: "In Progress",
      estimatedHours: 1.5,
      estimatedCost: 15000,
    ...
    {
      jobNumber: "JC-2026-005",
      appointmentId: appts[4].id,
      customerId: c2.customer.id,
      vehicleId: v4.id,
      branchId: mainBranch.id,
      description: "Full diagnostic scan — check engine, transmission, ABS",
      status: "Closed",
      estimatedHours: 3,
      estimatedCost: 45000,
      technicianId: tech1.id,
    ...
    {
      jobNumber: "JC-2026-006",
      appointmentId: appts[5].id,
      customerId: c3.customer.id,
      vehicleId: v5.id,
      branchId: phBranch.id,
    ...
    {
      jobNumber: "JC-2026-007",
      appointmentId: appts[6].id,
      customerId: c3.customer.id,
      vehicleId: v6.id,
      branchId: abujaBranch.id,
    ...
    {
      jobNumber: "JC-2026-008",
      appointmentId: appts[7].id,
      customerId: c4.customer.id,
      vehicleId: v7.id,
      branchId: mainBranch.id,
      description: "Spark plug replacement — all 4 cylinders",
      status: "Open",
      estimatedHours: 1,
      estimatedCost: 16200,
      technicianId: tech2.id,
    ...
    {
      jobNumber: "JC-2026-009",
      appointmentId: null,
      customerId: c0.customer.id,
      vehicleId: v0.id,
      branchId: phBranch.id,
      description: "Emergency — coolant leak investigation",
      status: "Closed",
      estimatedHours: 2,
      estimatedCost: 28000,
      technicianId: tech4.id,
...
async function seedInspectionsAndEstimates(
  ...
  for (const jc of closedCards) {
    ...
    if (!exists) {
      await prisma.inspection.create({
        data: {
          jobCardId: jc.id,
          inspectorId: advisor1.id,
          findings: `Vehicle inspected for ${jc.jobNumber}. All systems checked and within spec.`,
          passed: true,
          status: "Passed",
  ...
  const estimateData = [
    {
      idx: 0,
      description: "Oil change + brake inspection + air filter replacement",
      amount: 35000,
      status: "Approved",
    },
    {
      idx: 1,
      description: "Oil and filter change with 20-point inspection",
      amount: 22000,
      status: "Approved",
    },
    {
    ...
    {
      idx: 3,
      description: "Tyre rotation, balancing and alignment",
      amount: 15000,
      status: "Pending",
    },
    {
      idx: 4,
      description: "Full electronic diagnostic scan",
      amount: 45000,
      status: "Approved",
    },
    {
      idx: 5,
      description: "Front and rear brake pad set + labour",
      amount: 52000,
      status: "Approved",
    },
    {
      idx: 6,
      description: "Synthetic oil change — full synthetic 5W-30",
      amount: 12000,
      status: "Pending",
    },
    {
      idx: 7,
      description: "Spark plug set replacement (4 plugs) + labour",
      amount: 16200,
      status: "Pending",
    },
    {
      idx: 8,
      description: "Coolant system inspection and leak repair",
      amount: 28000,
      status: "Approved",
    },
  ];
  ...
  for (const e of estimateData) {
    const existing = await prisma.estimate.findFirst({
      where: { jobCardId: jobCards[e.idx].id, description: e.description },
    });
    const est =
      existing ??
      (await prisma.estimate.create({
        data: {
          jobCardId: jobCards[e.idx].id,
...
async function seedApprovalsPartIssuancesAndInvoices(
  ...
  for (const est of approvedEstimates) {
    const jc = await prisma.jobCard.findFirst({
      where: { estimates: { some: { id: est.id } } },
    });
    if (jc?.customerId) {
      const exists = await prisma.customerApproval.findFirst({
        where: { estimateId: est.id, customerId: jc.customerId },
      });
      if (!exists) {
        await prisma.customerApproval.create({
          data: {
            ...
            comments: "Approved via phone call",
            status: "Approved",
          },
        });
        approvalCount++;
  ...
  console.log(
    `✅ Seeded ${approvalCount} customer approvals (skipped existing)`,
  );
  ...
  for (const is of issuances) {
    ...
    if (jc) {
      const exists = await prisma.partIssuance.findFirst({
        where: { jobCardId: jc.id, sparePartId: is.partId },
      });
      if (!exists) {
        await prisma.partIssuance.create({
          data: {
            jobCardId: jc.id,
            sparePartId: is.partId,
            issuedById: advisor1.id,
            quantity: is.quantity,
            issuedAt: new Date(),
            notes: "Issued for job card repair",
          },
        });
        issuanceCount++;
      }
...
async function seedPaymentsAndReceipts(
  ...
  console.log(
    `✅ Seeded ${invoices.length} payments and ${invoices.length} receipts`,
  );
}
